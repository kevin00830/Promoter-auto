                                    ...
                                    <form action="{{route('main_web_inte')}}" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="row padflex askbox">
                                            <div class="col-md-6">
                                                <select class="form-control tmp_type_select" id="tmp_typebox" onchange="change_letoption(this.value)">
                                                    ...
                                                    <option value="6"> {{__('group.ask_for_email')}}</option>
                                                    <option value="7"> {{__('group.ask_for_street')}}</option>
                                                    <option value="8"> {{__('group.ask_for_number')}}</option>
                                                    <option value="9"> {{__('group.ask_for_complement')}}</option>
                                                    <option value="10"> {{__('group.ask_for_district')}}</option>
                                                    <option value="11"> {{__('group.ask_for_zipcode')}}</option>
                                                    <option value="12"> {{__('group.ask_for_city')}}</option>
                                                    <option value="13"> {{__('group.ask_for_state')}}</option>
                                                    <option value="14"> {{__('group.ask_for_country')}}</option>
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class=" hideall">
                                            ...
                                            <div class="col-md-6" id="" style="padding-bottom:5px !important">
                                                <input type="radio" value="0" id="actions1" name="actions" checked onchange="checkSelection3(this)">
                                            ...
                                            <div class="col-md-6" id="">
                                                ...
                                                <a href="#" onclick="reset_flow()" style="display:none;" id='recetbtn'> (<u>Reset current auto flow</u>)</a>
                                                <div id="appearNext" class="row padflex appear-next hidden">
                                                    <div class="col-md-6" style="padding-top:5px">
                                                        <?php //dd($flows); 8467 ?>
                                                        <select id="appearNext1" class="form-control auto_flow_select" name="auto_flow" id="autoflow">
                                    ...
                                    <table class="table table-separate table-hover table-checkable" id="kt_datatable_users22">
                                        ...
                                        @foreach($flows as $flow)
                                            <tr>
                                                ...
                                                <!--<td>{{$flow->main_msg}}</td>-->
                                                <td>
                                                    <button id="editBtn" class="btn btn-primary btn-sm edit-main-btn" data-id="{{$flow->id}}" data-fid="{{$flow->flow_id}}"> {{__('group.edit')}}</button>
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                if(tmp_type === '3' || tmp_type === '4' || tmp_type === '5' || tmp_type === '6' || tmp_type === '7' || tmp_type === '8' || tmp_type === '9' || tmp_type === '10' || tmp_type === '11' || tmp_type === '12' || tmp_type === '13' || tmp_type === '14') {
                                    ...
                                    $(".tmp_type_select").val(response['current_data'].tmp_type).trigger("change");
                                }
                                ...
                                if(response['current_data'].next === '1'){
                                    $('#actions2').prop('checked', true);
                                } else {
                                    $('#actions2').prop('checked', false);
                                }

                                var name = response['current_data'].is_name;
                                var empress = response['current_data'].is_empress;
                                var dob = response['current_data'].is_dob;
                                ...
                                // for (var i = 0; i < selectTag.options.length; i++) { // loop through each option
                                //     if (selectTag.options[i].value == autoFlow) { // check if the option's value is 2
                                //         selectTag.options[i].style.display = 'none'; // hide the option
                                //     }
                                // }
                            },
...
//   end of preview
            ...
            <script>
                function checkSelection(radio) {
                    $(".auto_flow_select").val("").trigger("change");
                    if (radio.checked) {
                        var div = document.getElementById('appearNext');
                        div.classList.remove('hidden');

                        // var selectTag = document.getElementById('appearNext1');
                        // var autoFlow = document.getElementById('editBtn').getAttribute('data-fid');
                ...
                function checkSelection3(radio) {
                    $(".auto_flow_select").val("").trigger("change");
                    if (radio.checked) {
                        var div = document.getElementById('appearNext');
            ...
            <script>
                function reset_flow(){
                    var selectedValue = $('#rest_flow_id').val();
                    $.post("{{ route('reset_flow') }}",
                        {
                            _token: csrfToken,
                            flow_id: selectedValue
                        },
